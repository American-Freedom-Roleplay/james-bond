...
-----------------------------------
-- Config --
webhookURL = 'https://discordapp.com/api/webhooks/798334075508490290/Ajj9GCZpQsfLWoLAsKyIYQQ3NuzCv5BE2q1MKp8JPbx1V71NT1u_WQbM4LXbWczDjHSc'
...
RegisterCommand("report", function(source, args, rawCommand)
    ...
    if #args < 2 then
    	TriggerClientEvent('chatMessage', source, "^1ERROR: Yanlış kullanım. ^2Doğrusu: /report <id> <sebep>")
    	return;
    end
    id = sm[2]
    if GetPlayerIdentifiers(id)[1] == nil then
    	TriggerClientEvent('chatMessage', source, "^1ERROR: Bu id ye uygun biri yok oyunda...")
    	return;
    end
	msg = ""
	local message = ""
	...
	if tonumber(id) ~= nil then
		...
		TriggerClientEvent('chatMessage', source, "^9[^1PARP^9] ^2Rapor görevlilere ulaşacak teşekkür ederiz :)")
		if not displayIdentifiers then 
			sendToDisc("YENİ RAPOR!: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 'Sebep: **' .. message ..
				'**', "Raporlayan kişi: [" .. source .. "] " .. GetPlayerName(source))
		else 
			...
			sendToDisc("DİKKAT YENİ RAPOR: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
				'Sebep: **' .. message ..
				'**\n' ..
				'Steam: **' .. steam .. '**\n' ..
				'Oyun lisansı: **' .. gameLicense .. '**\n' ..
				'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
				'Discord UID: **' .. discord:gsub('discord:', '') .. '**', "Reportlayan: [" .. source .. "] " .. GetPlayerName(source))
		end 
		--print("Runs report command fine and is number for ID") -- TODO - Debug
	else
		-- It's not a number
		TriggerClientEvent('chatMessage', source, "^9[^1PARP RAPOR^9] ^1Yanlış format. ^2Doğrusu: /report <id> <sebep>")
	end
end)
